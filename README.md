...
- **2.0.3** — 字幕下载、完成后自动关机；历史记录持久化（独立 history.json）并支持清空
- **2.0.2** — 修复「打开文件」找不到合并后输出；新增下载历史 + 一键重下载；exe 图标与单实例限制；README 增加 AV1 解码提示
- **2.0.1** — 清理旧 Web 相关文件；打包改为 onedir 文件夹版，启动更快
- **2.0.0** — 桌面化重构：去掉 Flask/Web UI，现代化 QML 界面，内置 FFmpeg
